...
// =============================================================================
// Shidduch Organizer — Single File App (clean, fully updated)
// =============================================================================
const APP_VERSION = "v2.0 (Lite)";
...
const shareRef = async (ref, label = "file") => {
  try {
    ...
    if (navAny.share && navAny.canShare && navAny.canShare({ files: [file] })) {
      try { await navAny.share({ files: [file], title: fileName, text: `Sharing ${label}` }); return; }
      catch (e) { if (e?.name === "AbortError") return; }
    }

    const url = URL.createObjectURL(blob);
    if (navAny.share) {
      try { await navAny.share({ url, title: fileName, text: `Sharing ${label}` }); setTimeout(() => URL.revokeObjectURL(url), 15000); return; }
      catch (_) {}
    }
    ...
    try { await downloadRef(ref); } catch {}
  }
};
...
const shareKidAll = async (kid) => {
  ...
  try {
    ...
    alert("Share failed. You can still Export from the top toolbar.");
  }
};
...
// ===== Upload previews (stable, cached) =====
const __urlCache = new Map(); // id -> { url, ts }
  ...
  return (
    <div className="h-28 border rounded-lg bg-white p-2 flex flex-col">
      <div
        ...
        title="Tap to view"
      >
        {url ? (<span className="text-xs text-gray-500 truncate">{file.name}</span>) : (<span className="text-xs text-gray-400">Loading…</span>)}
      </div>
...
function ViewerModal({ open, fileRef, onClose }) {
  const [url, setUrl] = useState("");
  const type = fileRef?.type || "";
  useEffect(() => {
    let alive = true; let obj = "";
    (async () => {
      if (!open || !fileRef?.id) { setUrl(""); return; }
      const blob = await dbFiles.getItem(fileRef.id);
      if (!alive || !blob) return;
      obj = URL.createObjectURL(blob);
      setUrl(obj);
    })();
    return () => { alive = false; if (obj) try { URL.revokeObjectURL(obj); } catch {} };
  }, [open, fileRef?.id]);
  ...
  return (
    <div className="fixed inset-0 z-[999] bg-black/70 backdrop-blur-sm flex flex-col" onClick={onClose}>
      <div className="flex justify-between items-center px-4 py-3 bg-black/40 text-white" onClick={(e)=>e.stopPropagation()}>
        <div className="text-sm truncate">{fileRef?.name || "Preview"}</div>
        <div className="flex gap-2">
          <button className="px-3 py-1 rounded border border-white/40 hover:bg-white/10 text-sm" onClick={(e)=>{ e.stopPropagation(); shareRef(fileRef, "file"); }}>Share</button>
          <button className="px-3 py-1 rounded border border-white/40 hover:bg-white/10 text-sm" onClick={(e)=>{ e.stopPropagation(); downloadRef(fileRef); }}>Download</button>
          <button className="px-3 py-1 rounded bg-white text-black text-sm" onClick={onClose}>Close</button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden" onClick={(e)=>e.stopPropagation()}>
        {isImg && url ? (
          <div className="w-full h-full flex items-center justify-center p-2">
            <img src={url} alt="preview" className="max-w-[100vw] max-h-[calc(100vh-80px)] object-contain rounded" />
          </div>
        ) : isPdf && url ? (
          <iframe title="PDF preview" src={url} className="w-full h-full border-0 bg-white" />
        ) : url ? (
          <div className="w-full h-full flex items-center justify-center">
            <a href={url} target="_blank" rel="noreferrer" className="px-4 py-2 rounded bg-white">Open in new tab</a>
          </div>
        ) : (
          <div className="w-full h-full flex items-center justify-center text-white/80">Loading…</div>
        )}
      </div>
    </div>
  );
...
// ===== Prospects =====
function Prospects({ prospects, setProspects, profile, saveProfile, activeKidId, setActiveKidId, openViewer }){
  ...
  // Clipboard API try, else visible paste box
        ...
        {filtered.map(p=> (
          <div key={p.id} className="border rounded bg-white shadow-sm" onDragOver={(e)=>e.preventDefault()} onDrop={async(e)=>{ e.preventDefault(); await onDropFiles(p.id, e.dataTransfer?.files||null); }}>
            <div className="p-2 flex items-center gap-2">
              <EditableText value={p.fullName} onChange={(v)=>updateP(p.id,{fullName:v||"Untitled"})} className="font-medium truncate flex-1 text-left" inputClass="font-medium truncate flex-1 border rounded px-2 py-1" />
              ...
              <button type="button" aria-label="Delete prospect" className="w-7 h-7 rounded-full border border-rose-300 text-rose-700 flex items-center justify-center hover:bg-rose-50" onClick={()=>removeP(p.id)}>×</button>
            </div>
            {expanded[p.id] && (
              <div className="p-2 pt-0">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  <UploadBox emptyLabel="Add PDF"   accept="application/pdf" file={p.resume} onPick={(ref)=>updateP(p.id,{resume:ref})}
                    ...
                    onOpen={(ref)=>openViewer(ref)}
                  />
                  <UploadBox emptyLabel="Add photo" accept="image/*"         file={p.photo}  onPick={(ref)=>updateP(p.id,{photo:ref})}
                ...
                <div className="mt-2">
                  <button type="button" className="px-3 py-1 rounded-full border text-xs bg-white hover:bg-blue-50 border-blue-300 text-blue-700 disabled:opacity-50"
                    onClick={()=>shareKidAll({ name:(kids.find(k=>k.id===p.kidId)?.name)||p.fullName, photo:p.photo, resume:p.resume, blurb:(kids.find(k=>k.id===p.kidId)?.blurb)||"" })}
                    disabled={!(((kids.find(k=>k.id===p.kidId)?.blurb||"").trim()) || p.photo || p.resume)}
                  >
                    Share
                  </button>
                </div>
...
// ===== My Info =====
function MyInfo({ profile, saveProfile, openViewer }){
  const kids=ensureArray(profile?.kids);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 flex-wrap">
        {kids.map(k=> (
          editKidId===k.id ? (
            ...
            <button key={k.id} className={`px-3 py-1 rounded-full border ${selId===k.id?"bg-black text-white":"bg-white"}`}
              onClick={()=>{ if(selId!==k.id) setSelId(k.id); else { setEditKidId(k.id); setEditVal(k.name||""); } }}
              onContextMenu={(e)=>{ e.preventDefault(); const r=e.currentTarget.getBoundingClientRect(); setKidMenu({open:true,kidId:k.id,x:r.left+r.width/2,y:r.bottom+8}); }}
              onMouseDown={(e)=>startLP(e.currentTarget,k.id)} onMouseUp={cancelLP} onMouseLeave={cancelLP}
              onTouchStart={(e)=>startLP(e.currentTarget,k.id)} onTouchEnd={cancelLP} onTouchMove={cancelLP}
            >{k.name||"Unnamed"}</button>
          )
        ))}
        <button className="px-3 py-1 rounded-full border" onClick={addKid} aria-label="Add child">+</button>
      </div>
      ...
      {kidMenu.open && (
        <div ref={menuRef} style={{position:"fixed", left:kidMenu.x, top:kidMenu.y, transform:"translateX(-50%)"}} className="z-50 rounded border bg-white shadow">
      ...
      {selected ? (
        ...
        <div className="text-xs text-gray-500">Add a child to attach a photo or resume.</div>
      )}
...
export default function App(){
  ...
  const openViewer = (ref) => { setViewerRef(ref); setViewerOpen(true); };
  const closeViewer = () => { setViewerOpen(false); setViewerRef(null); };
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto text-sm">
      <h1 className="text-xl font-semibold mb-3">Shidduch Organizer • {APP_VERSION}</h1>

      <div className="flex gap-2 mb-4 items-center">
        <button className={`px-3 py-1 rounded border ${tab==="prospects"?"bg-black text-white":"bg-white"}`} onClick={()=>setTab("prospects")}>Resumes</button>
        <AddDropdown />
        <div className="ml-auto flex gap-2 items-center">
          <button className={`px-3 py-1 rounded border ${tab==="profile"?"bg-black text-white":"bg-white"}`} onClick={()=>setTab("profile")}>My Info</button>
          <input ref={importRef} type="file" accept="application/json" className="hidden" onChange={e=>{ const f=e.target.files?.[0]; if(f) importAll(f); }} />
        </div>
      </div>

      <SettingsFab onExport={exportAll} onImport={() => importRef.current?.click()} onOpenSync={()=>setSyncOpen(true)} />

      <SyncPanel open={syncOpen} initial={sync} onSave={saveSync} onClear={clearSync} onClose={()=>setSyncOpen(false)} />
      ...
      {tab==="prospects" ? (
        ...
        <MyInfo profile={profile} saveProfile={saveProfile} openViewer={openViewer} />
      )}

      <ViewerModal open={viewerOpen} fileRef={viewerRef} onClose={closeViewer} />
    </div>
  );
}
